...
class NSGAII:
    ...
    def __init__(
        ...
        self.accessible = instance.accessible
        ...
        # 自适应小生境参数
        self.min_diversity_threshold = 0.001  # 最小多样性阈值(后期,允许更接近)
        self.max_diversity_threshold = 0.05  # 最大多样性阈值(早期,严格互斥)
        self.max_niche_capacity = 3  # 每个小生境最大容量(允许有限堆积)
    ...
    def _rebuild_route_from_tasks(
        ...
        Returns:
            重建后的解
        """
        comm_coverage_matrix = self.comm_coverage_matrix[veh_id]
        is_ground_veh = veh_id in self.instance.ground_veh_ids
        accessible_list = self.accessible
    ...
    def _improve_coverage(
        ...
        # 提取必要信息
        comm_coverage_matrix = self.instance.comm_coverage_matrix
        accessible_list = self.instance.accessible
        is_ground_veh = modified_veh in self.instance.ground_veh_ids

        # 步骤1: 收集全体已服务的需求节点，和未服务的需求节点
        included_tasks = solution.get_included_nodes()
        unincluded_tasks = set(self.id_demand) - included_tasks

        # 获取修改车辆的路径
        route = solution.routes[modified_veh]

        # 步骤2: 扩充coverage - 对路径上每个节点,添加其能覆盖但未被其他车辆覆盖的需求
        for visit_node in route[1:-1]:  # 排除起终点depot
            key = (modified_veh, visit_node)

            # 如果是地面车辆且节点不可达,跳过
            if is_ground_veh and accessible_list[visit_node] == 0:
                continue

            # 找出该节点可以覆盖的所有需求节点(使用通信覆盖矩阵)
            covered_mask = comm_coverage_matrix[modified_veh][visit_node]
            all_coverable = np.where(covered_mask)[0].tolist()

            # 只保留需求节点(排除depot和steiner节点)
            all_coverable = [t for t in all_coverable if t in unincluded_tasks]

            # 只添加尚未被覆盖的需求节点
            new_covered = set(all_coverable)

            if new_covered:
                # 如果该节点已有coverage记录,则扩充
                if key in solution.coverage:
                    solution.coverage[key].update(new_covered)
                else:
                    raise ValueError("路径中的访问没有对应的coverage信息")

                # 更新已覆盖集合
                included_tasks.update(new_covered)
                unincluded_tasks -= new_covered

        # # debug: 检查是否出现新空解
        # if self._is_empty_solution(solution):
        #     print(
        #         "-------------------------------------"
        #         f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}: "
        #         f"{self._is_empty_solution(solution)}"
        #         "improve后，产生空子代子代解"
        #     )
    ...
    def _cross_vehicle_mutation(self, solution: Solution, mutate_type: str) -> Solution:
        ...
        # todo: 如果两个车辆的覆盖半径不一样，需要考虑coverage信息的变化
        comm_coverage_matrix = self.instance.comm_coverage_matrix

        for k, v in route_coverage1.items():
            veh_id, node = k
            covered_mask = comm_coverage_matrix[veh1][node]
            all_coverable = np.where(covered_mask)[0].tolist()
            all_coverable = set([t for t in all_coverable if t in self.id_demand])
            route_coverage1[k] = set(
                [task for task in route_coverage1[k] if task in all_coverable]
            )
            if route_coverage1[k]:
                solution.coverage[k] = route_coverage1[k]
            else:
                del solution.coverage[k]
                for visit in solution.routes[veh_id]:
                    if visit == node:
                        solution.routes[veh_id].remove(visit)
                        break

        for k, v in route_coverage2.items():
            veh_id, node = k
            covered_mask = comm_coverage_matrix[veh2][node]
            all_coverable = np.where(covered_mask)[0].tolist()
            route_coverage2[k] = set(
                [task for task in route_coverage2[k] if task in all_coverable]
            )
            if route_coverage2[k]:
                solution.coverage[k] = route_coverage2[k]
            else:
                del solution.coverage[k]
                for visit in solution.routes[veh_id]:
                    if visit == node:
    ...
    def _task_level_mutation(self, solution: Solution, mutate_type: str) -> Solution:
        ...
        if mutate_type == "new_mission":
            # 任务形式替换
            node_included = mutated_solution.get_included_nodes()
            all_tasks = set(self.id_demand) - node_included
            if all_tasks:
                new_task = random.choice(list(all_tasks))
                # 先检查是否可以被现在的点覆盖
                coverage = mutated_solution.coverage
                can_be_covered = False
                for (veh_id, visit_node), covered_set in coverage.items():
                    coverable_flag = self.comm_coverage_matrix[
                        veh_id, visit_node, new_task
                    ]
                    if coverable_flag:
                        # 可以被覆盖
                        covered_set.add(new_task)
                        can_be_covered = True
                        break

                if not can_be_covered:
                    accessible = self.instance.accessible[new_task]
                    if accessible:
                        veh_id = random.choice(
                            list(self.instance.ground_veh_ids)
                            + list(self.instance.drone_ids)
                        )
                    else:
                        veh_id = random.choice(list(self.instance.drone_ids))
                    # 随机选择一个位置
                    current_route = mutated_solution.routes[veh_id]
                    insert_pos = random.randint(1, len(current_route) - 1)
                    # 插入新任务节点
                    current_route.insert(insert_pos, new_task)
                    # 更新覆盖信息
                    mutated_solution.coverage[(veh_id, new_task)] = {new_task}
                    mutated_solution = self._improve_coverage(mutated_solution, veh_id)

                    # # debug: 检查解是否可行
                    if self.debug_mode:
                        self.population.evaluator.sol_feasible(mutated_solution)

        elif mutate_type == "reselection":

            # 随机选择一个有路径的车辆
            vehicle_ids = list(self.instance.ground_veh_ids) + list(
                self.instance.drone_ids
            )
            ...
            if better_node is not None and better_node != visit_node:
                if better_node in mutated_solution.routes[selected_veh]:
                    ...
                    # 更新覆盖信息
                    if (selected_veh, visit_node) in mutated_solution.coverage:
                        old_coverage = mutated_solution.coverage[
                            (selected_veh, visit_node)
                        ]
                        # 通过coverage_matrix判断该车在这里是否可以覆盖该点
                        comm_coverage_array = self.comm_coverage_matrix[selected_veh][
                            better_node
                        ]
                        reach_able_set = set(np.where(comm_coverage_array)[0].tolist())
                        old_coverage = old_coverage & reach_able_set
                        del mutated_solution.coverage[(selected_veh, visit_node)]
                ...
                for k, v in mutated_solution.coverage.items():
                    if len(v) == 0:
                        coverage_to_del.append(k)
                        veh_id, node_id = k
                        for visit in mutated_solution.routes[veh_id]:
                            if visit == node_id:
                                mutated_solution.routes[veh_id].remove(visit)
                                break
                for k in coverage_to_del:
                    del mutated_solution.coverage[k]
                ...
                # 调整覆盖信息
                mutated_solution = self._improve_coverage(
                    mutated_solution, selected_veh
                )
