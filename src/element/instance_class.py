...
class InstanceClass:
    ...
    def __post_init__(self):
        ...
        # 计算总数，方便调用
        self.total_node_num = self.base_num + self.demand_num + self.steiner_num
        ...
        ground_capacity = vehicle_config.ground_veh_energy_capacity
        drone_capacity = vehicle_config.drone_energy_capacity
        self.capacity_array: np.ndarray = np.array(
            [ground_capacity] * self.ground_veh_num + [drone_capacity] * self.drone_num
        )
        ground_comm_radius = vehicle_config.ground_veh_comm_radius
        drone_comm_radius = vehicle_config.drone_comm_radius
        self.comm_radius_array: np.ndarray = np.array(
            [ground_comm_radius] * self.ground_veh_num
            + [drone_comm_radius] * self.drone_num
        )
        ...
        # 预处理：计算行驶能耗矩阵、通信能耗矩阵
        self.travel_consum_matrix = compute_travel_consum_matrix(self)
        self.comm_consum_matrix = compute_comm_consum_matrix(self)

    def __str__(self) -> str:
        str_info = (
            f"算例名称: {self.name} "
            f"(需求点 = {self.demand_num}个, "
            f"Steiner点 = {self.steiner_num}个, "
    ...
    def new_instance(
        ...
        base_ids, demand_ids, steiner_ids = allocate_ids_np(
            0, base_num, demand_num, steiner_num
        )
        ground_veh_ids, drone_ids = allocate_ids_np(0, ground_veh_num, drone_num)

        com_time = prob_config.instance_param.min_comm_time

        min_com_time = [0.0] * base_num + [com_time] * demand_num + [0.0] * steiner_num

        return cls(
            name=name,
            base_num=base_num,
            demand_num=demand_num,
            steiner_num=steiner_num,
            ground_veh_num=ground_veh_num,
            drone_num=drone_num,
            base_ids=base_ids,
            demand_ids=demand_ids,
            steiner_ids=steiner_ids,
...
def compute_distance_matrix(instance: InstanceClass) -> np.ndarray:
    """计算距离矩阵"""
    np_coords = np.array(instance.coords)
    # 使用广播机制一次性计算所有距离
    diff = np_coords[:, np.newaxis, :] - np_coords[np.newaxis, :, :]
    dist_matrix = np.sqrt(np.sum(diff**2, axis=-1))
    round_dist_matrix: np.ndarray = np.round(dist_matrix, 2)
    return round_dist_matrix
...
def compute_coverage_matrix(instance: InstanceClass):
    ...
    - 车辆v在节点i(demand或steiner)时,能否覆盖节点j(仅demand)
    - i 可以是 demand_ids 或 steiner_ids 中的点(执行覆盖的位置)
    ...
    coverage_matrix = np.zeros(
        (total_veh_num, total_node_num, total_node_num), dtype=bool
    )

    for v in range(total_veh_num):
        if v in instance.ground_veh_ids:
            cover_radius = config.instance_param.vehicle_config.ground_veh_comm_radius
        ...
        # 发出信号的点: demand_ids + steiner_ids
        for i in demand_ids + steiner_ids:
            # 被覆盖的点: 仅 demand_ids
            for j in demand_ids:
                coverage_matrix[v, i, j] = instance.distance_matrix[i, j] < cover_radius
...
def compute_travel_time_matrix(instance: InstanceClass) -> np.ndarray:
    ...
    # 初始化 travel_time_matrix为0
    travel_time_matrix = np.zeros(
        (total_veh_num, instance.total_node_num, instance.total_node_num)
    )
    ...
    for v in range(total_veh_num):
        if v in ground_veh_ids:
            ...
            if speed > 0:
                ...
                travel_time_matrix[v][inaccessible_mask] = big_M
            else:
